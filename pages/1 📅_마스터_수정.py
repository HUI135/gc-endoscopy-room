...
/* [변경] CSS Grid를 사용한 캘린더 전체 컨테이너 */
.calendar-grid {
    display: grid;
    grid-template-columns: repeat(7, 1fr); /* 항상 7개의 동일한 너비의 열을 유지 */
    gap: 5px; /* 셀 사이의 간격 */
}
/* 요일 헤더 */
.calendar-header {
...
.calendar-day-cell {
    ...
    border-radius: 5px;
    padding: 6px;
    min-height: 110px; /* 모바일에서도 적절한 높이 유지 */
...
.day-number {
    font-weight: bold;
    font-size: 0.8rem; /* 모바일을 위해 상대 단위로 변경 */
    margin-bottom: 5px;
}
/* 다른 달의 날짜는 회색으로 */
...
/* 이벤트 아이템 스타일 */
.event-item {
    font-size: 0.7rem; /* 모바일을 위해 상대 단위로 변경 */
    padding: 2px 4px;
    border-radius: 3px;
    margin-bottom: 3px;
    color: white;
...
# 날짜별 이벤트 가공
events_by_date = {}
# `events` 변수는 코드 상단 `generate_calendar_events` 함수에서 이미 생성되었습니다.
for event in events:
...
for day in days_of_week:
    ...
    if day == "토":
        ...
        header_class += " sunday"
    calendar_html += f'<div class="{header_class}">{day}</div>'
...
for week in month_days:
    for day_date in week:
        ...
        """
calendar_html += '</div>' # calendar-grid div 닫기

# 최종적으로 생성된 HTML을 한 번에 출력
