...
def random_assign(personnel, slots, request_assignments, time_groups, total_stats, morning_personnel, afternoon_personnel, afternoon_duty_counts):
    ...
    for person in morning_remaining:
        ...
        for slot_idx in morning_slot_indices:
            ...
            slot = slots[slot_idx]
            room_num = slot.split('(')[1].split(')')[0]
            room_count = total_stats['rooms'][room_num][person] + daily_stats['rooms'][room_num][person] + 1
            score = room_count  # 방별 배정 균형만 고려
            if score < min_score:
                min_score = score
                best_slot_idx = slot_idx
        
        if best_slot_idx is None:
            # 사용 가능한 첫 번째 슬롯 강제 선택
            for idx in morning_slot_indices:
                if assignment[idx] is None:
                    best_slot_idx = idx
                    break
            st.warning(f"오전 인원 {person} 강제 배정: 오전 슬롯 {slots[best_slot_idx]}")
    ...
    random.shuffle(afternoon_remaining)
    for person in afternoon_remaining:
        ...
        if best_slot_idx is None:
            # 사용 가능한 첫 번째 슬롯 강제 선택
            for idx in afternoon_slot_indices:
                if assignment[idx] is None:
                    best_slot_idx = idx
                    break
            st.warning(f"오후 인원 {person} 강제 배정: 오후 슬롯 {slots[best_slot_idx]}")
    ...
    for slot_idx in range(len(slots)):
        if assignment[slot_idx] is None:
            ...
            if not available_personnel:
                # 이미 배정된 인원 중에서 선택
                available_personnel = morning_personnel if slot in morning_slots else afternoon_personnel
                random.shuffle(available_personnel)
