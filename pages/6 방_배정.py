...
# 변경된 세션 상태 변수를 사용하여 로그 출력
change_log_list = st.session_state.get("swapped_assignments_log", [])
if change_log_list:
    log_df = pd.DataFrame(change_log_list)
    log_df = log_df.sort_values(by=['날짜', '근무타입']).reset_index(drop=True)
